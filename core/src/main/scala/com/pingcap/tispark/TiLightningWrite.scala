...
import com.pingcap.tispark.TiLightningWrite.{SparkRow, TiHandle, TiRow}
import com.pingcap.tispark.write.{SerializableKey, TiDBOptions, WrappedRow}
...
object TiLightningWrite {
  ...
  type TiHandle = com.pingcap.tikv.key.Handle
...
class TiLightningWrite(
  ...
  private def doWrite(): Unit = {
    ...
    val rddAfterSplit: RDD[WrappedRow] = tiRowRdd
      ...
      .map(x => WrappedRow(x._1, new IntHandle(x._2)))
  ...
  private def doImport(iter: Iterator[WrappedRow]): Unit = {
    ...
    val uuid = UUIDType5
      .nameUUIDFromNamespaceAndString(
        UUID.fromString("d68d6abe-c59e-45d6-ade8-e2b0ceb7bedf"),
        tag)
  ...
  private def generateRowKey(row: TiRow, handle: TiHandle): (SerializableKey, Array[Byte]) = {
    (
  ...
  private def encodeTiRow(tiRow: TiRow): Array[Byte] = {
    ...
    TableCodec.encodeRow(tiTableInfo.getColumns, convertedValues, tiTableInfo.isPkHandle, true)
  }
}
